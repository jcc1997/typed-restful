...
export function defineRoute<
  M extends Method,
  R extends string,
  Cb extends (request: any) => any
>(method: M, route: R, cb: Cb): PathRecord<M, RemoveSlash<R>, Cb> {
  throw new Error("not implement");
}

export function createService<T extends Record<string, any>>(routes: T): Service<T> {
  throw new Error("not implement");
}
...
//     };
// }
const route1 = defineRoute("GET", "/topic/:id", (request: string) => { return 123; });
...
> = Extract<keyof T, Method> extends never
    ? { [K in keyof Sub]: K extends string ? Flatten<Sub[K], `${L}/${Convert$<K>}`> : never }[keyof Sub]
    : { [LK in L]: Ms} | { [K in keyof Sub]: K extends string ? Flatten<Sub[K], `${L}/${Convert$<K>}`> : never }[keyof Sub];
type FlattenRoutes<T extends Record<string, any>> = IntersectionFromUnion<Flatten<T>>;
...
type Service<T extends Record<string, any>, FRoutes = FlattenRoutes<T>> = {
  ...
  Req extends Handler extends (...args: any) => any ? Parameters<Handler>[0] : never,
  Resp extends Handler extends (...args: any) => any ? ReturnType<Handler> : never,
  Overlap = RoutePattern<R> & keyof FRoutes,
  Handlers = Overlap extends keyof FRoutes ? FRoutes[Overlap] : never,
  Handler = Handlers[M],
  >(method: M, url: R, request: Req): Promise<Resp>;
};
